...
from src.app.user.user_manager import get_user_manager
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_manager = get_user_manager(vector_store)
    ...
    if user_profile is None:
        ...
        if user_profile.first_name:
            welcome_msg = f"👋 {user_profile.first_name}，欢迎回来！\n\n" + WELCOME_RETURNING.split('\n\n', 1)[1]
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        response = generate_response(user_input, llm=llm, vector_store=vector_store, user_id=user_id)
        ...
        # 合规日志：记录机器人响应
        audit_logger.log_bot_response(user_id, response, processing_time, len(response))
        
        # 传统日志：只记录成功信息
        logger.info(f"Response sent to user {user_id}, processing time: {processing_time:.1f}ms")
        
    except Exception as e:
        error_msg = "抱歉，处理您的问题时出现了错误。请稍后再试，或联系我们的客服：contact@thinkforward.ai"
