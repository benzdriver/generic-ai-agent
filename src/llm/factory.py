...
class LLMFactory:
    ...
    @classmethod
    def get_llm(cls, provider: str = "openai", fallback_providers: Optional[List[str]] = None) -> BaseLLM:
        """获取LLM实例
        ...
        try:
            if provider not in cls._instances:
                ...
                cls._instances[provider] = cls._implementations[provider]()
            ...
            try:
                if fallback not in cls._instances:
                    if fallback not in cls._implementations:
                        continue
                    cls._instances[fallback] = cls._implementations[fallback]()
...
class FallbackLLM(BaseLLM):
    ...
    def __init__(self, primary_provider: str = "openai", fallback_providers: Optional[List[str]] = None):
